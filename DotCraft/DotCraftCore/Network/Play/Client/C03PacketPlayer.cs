namespace DotCraftCore.Network.Play.Client
{

	using INetHandler = DotCraftCore.Network.INetHandler;
	using Packet = DotCraftCore.Network.Packet;
	using PacketBuffer = DotCraftCore.Network.PacketBuffer;
	using INetHandlerPlayServer = DotCraftCore.Network.Play.INetHandlerPlayServer;

	public class C03PacketPlayer : Packet
	{
		protected internal double field_149479_a;
		protected internal double field_149477_b;
		protected internal double field_149478_c;
		protected internal double field_149475_d;
		protected internal float field_149476_e;
		protected internal float field_149473_f;
		protected internal bool field_149474_g;
		protected internal bool field_149480_h;
		protected internal bool field_149481_i;
		

		public C03PacketPlayer()
		{
		}

		public C03PacketPlayer(bool p_i45256_1_)
		{
			this.field_149474_g = p_i45256_1_;
		}

		public virtual void processPacket(INetHandlerPlayServer p_148833_1_)
		{
			p_148833_1_.processPlayer(this);
		}

///    
///     <summary> * Reads the raw packet data from the data stream. </summary>
///     
//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void readPacketData(PacketBuffer p_148837_1_) throws IOException
		public override void readPacketData(PacketBuffer p_148837_1_)
		{
			this.field_149474_g = p_148837_1_.readUnsignedByte() != 0;
		}

///    
///     <summary> * Writes the raw packet data to the data stream. </summary>
///     
//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writePacketData(PacketBuffer p_148840_1_) throws IOException
		public override void writePacketData(PacketBuffer p_148840_1_)
		{
			p_148840_1_.writeByte(this.field_149474_g ? 1 : 0);
		}

		public virtual double func_149464_c()
		{
			return this.field_149479_a;
		}

		public virtual double func_149467_d()
		{
			return this.field_149477_b;
		}

		public virtual double func_149472_e()
		{
			return this.field_149478_c;
		}

		public virtual double func_149471_f()
		{
			return this.field_149475_d;
		}

		public virtual float func_149462_g()
		{
			return this.field_149476_e;
		}

		public virtual float func_149470_h()
		{
			return this.field_149473_f;
		}

		public virtual bool func_149465_i()
		{
			return this.field_149474_g;
		}

		public virtual bool func_149466_j()
		{
			return this.field_149480_h;
		}

		public virtual bool func_149463_k()
		{
			return this.field_149481_i;
		}

		public virtual void func_149469_a(bool p_149469_1_)
		{
			this.field_149480_h = p_149469_1_;
		}

		public override void processPacket(INetHandler p_148833_1_)
		{
			this.processPacket((INetHandlerPlayServer)p_148833_1_);
		}

		public class C04PacketPlayerPosition : C03PacketPlayer
		{
			

			public C04PacketPlayerPosition()
			{
				this.field_149480_h = true;
			}

			public C04PacketPlayerPosition(double p_i45253_1_, double p_i45253_3_, double p_i45253_5_, double p_i45253_7_, bool p_i45253_9_)
			{
				this.field_149479_a = p_i45253_1_;
				this.field_149477_b = p_i45253_3_;
				this.field_149475_d = p_i45253_5_;
				this.field_149478_c = p_i45253_7_;
				this.field_149474_g = p_i45253_9_;
				this.field_149480_h = true;
			}

//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void readPacketData(PacketBuffer p_148837_1_) throws IOException
			public override void readPacketData(PacketBuffer p_148837_1_)
			{
				this.field_149479_a = p_148837_1_.readDouble();
				this.field_149477_b = p_148837_1_.readDouble();
				this.field_149475_d = p_148837_1_.readDouble();
				this.field_149478_c = p_148837_1_.readDouble();
				base.readPacketData(p_148837_1_);
			}

//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writePacketData(PacketBuffer p_148840_1_) throws IOException
			public override void writePacketData(PacketBuffer p_148840_1_)
			{
				p_148840_1_.writeDouble(this.field_149479_a);
				p_148840_1_.writeDouble(this.field_149477_b);
				p_148840_1_.writeDouble(this.field_149475_d);
				p_148840_1_.writeDouble(this.field_149478_c);
				base.writePacketData(p_148840_1_);
			}

			public override void processPacket(INetHandler p_148833_1_)
			{
				base.processPacket((INetHandlerPlayServer)p_148833_1_);
			}
		}

		public class C05PacketPlayerLook : C03PacketPlayer
		{
			

			public C05PacketPlayerLook()
			{
				this.field_149481_i = true;
			}

			public C05PacketPlayerLook(float p_i45255_1_, float p_i45255_2_, bool p_i45255_3_)
			{
				this.field_149476_e = p_i45255_1_;
				this.field_149473_f = p_i45255_2_;
				this.field_149474_g = p_i45255_3_;
				this.field_149481_i = true;
			}

//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void readPacketData(PacketBuffer p_148837_1_) throws IOException
			public override void readPacketData(PacketBuffer p_148837_1_)
			{
				this.field_149476_e = p_148837_1_.readFloat();
				this.field_149473_f = p_148837_1_.readFloat();
				base.readPacketData(p_148837_1_);
			}

//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writePacketData(PacketBuffer p_148840_1_) throws IOException
			public override void writePacketData(PacketBuffer p_148840_1_)
			{
				p_148840_1_.writeFloat(this.field_149476_e);
				p_148840_1_.writeFloat(this.field_149473_f);
				base.writePacketData(p_148840_1_);
			}

			public override void processPacket(INetHandler p_148833_1_)
			{
				base.processPacket((INetHandlerPlayServer)p_148833_1_);
			}
		}

		public class C06PacketPlayerPosLook : C03PacketPlayer
		{
			

			public C06PacketPlayerPosLook()
			{
				this.field_149480_h = true;
				this.field_149481_i = true;
			}

			public C06PacketPlayerPosLook(double p_i45254_1_, double p_i45254_3_, double p_i45254_5_, double p_i45254_7_, float p_i45254_9_, float p_i45254_10_, bool p_i45254_11_)
			{
				this.field_149479_a = p_i45254_1_;
				this.field_149477_b = p_i45254_3_;
				this.field_149475_d = p_i45254_5_;
				this.field_149478_c = p_i45254_7_;
				this.field_149476_e = p_i45254_9_;
				this.field_149473_f = p_i45254_10_;
				this.field_149474_g = p_i45254_11_;
				this.field_149481_i = true;
				this.field_149480_h = true;
			}

//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void readPacketData(PacketBuffer p_148837_1_) throws IOException
			public override void readPacketData(PacketBuffer p_148837_1_)
			{
				this.field_149479_a = p_148837_1_.readDouble();
				this.field_149477_b = p_148837_1_.readDouble();
				this.field_149475_d = p_148837_1_.readDouble();
				this.field_149478_c = p_148837_1_.readDouble();
				this.field_149476_e = p_148837_1_.readFloat();
				this.field_149473_f = p_148837_1_.readFloat();
				base.readPacketData(p_148837_1_);
			}

//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writePacketData(PacketBuffer p_148840_1_) throws IOException
			public override void writePacketData(PacketBuffer p_148840_1_)
			{
				p_148840_1_.writeDouble(this.field_149479_a);
				p_148840_1_.writeDouble(this.field_149477_b);
				p_148840_1_.writeDouble(this.field_149475_d);
				p_148840_1_.writeDouble(this.field_149478_c);
				p_148840_1_.writeFloat(this.field_149476_e);
				p_148840_1_.writeFloat(this.field_149473_f);
				base.writePacketData(p_148840_1_);
			}

			public override void processPacket(INetHandler p_148833_1_)
			{
				base.processPacket((INetHandlerPlayServer)p_148833_1_);
			}
		}
	}

}