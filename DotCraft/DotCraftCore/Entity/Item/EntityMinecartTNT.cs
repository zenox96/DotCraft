using System;

namespace DotCraftCore.nEntity.nItem
{

	using Block = DotCraftCore.nBlock.Block;
	using BlockRailBase = DotCraftCore.nBlock.BlockRailBase;
	using Blocks = DotCraftCore.nInit.Blocks;
	using ItemStack = DotCraftCore.nItem.ItemStack;
	using NBTTagCompound = DotCraftCore.nNBT.NBTTagCompound;
	using DamageSource = DotCraftCore.nUtil.DamageSource;
	using Explosion = DotCraftCore.nWorld.Explosion;
	using World = DotCraftCore.nWorld.World;

	public class EntityMinecartTNT : EntityMinecart
	{
		private int minecartTNTFuse = -1;
		

		public EntityMinecartTNT(World p_i1727_1_) : base(p_i1727_1_)
		{
		}

		public EntityMinecartTNT(World p_i1728_1_, double p_i1728_2_, double p_i1728_4_, double p_i1728_6_) : base(p_i1728_1_, p_i1728_2_, p_i1728_4_, p_i1728_6_)
		{
		}

		public override int MinecartType
		{
			get
			{
				return 3;
			}
		}

		public override Block func_145817_o()
		{
			return Blocks.tnt;
		}

///    
///     <summary> * Called to update the entity's position/logic. </summary>
///     
		public override void onUpdate()
		{
			base.onUpdate();

			if (this.minecartTNTFuse > 0)
			{
				--this.minecartTNTFuse;
				this.worldObj.spawnParticle("smoke", this.posX, this.posY + 0.5D, this.posZ, 0.0D, 0.0D, 0.0D);
			}
			else if (this.minecartTNTFuse == 0)
			{
				this.explodeCart(this.motionX * this.motionX + this.motionZ * this.motionZ);
			}

			if (this.isCollidedHorizontally)
			{
				double var1 = this.motionX * this.motionX + this.motionZ * this.motionZ;

				if (var1 >= 0.009999999776482582D)
				{
					this.explodeCart(var1);
				}
			}
		}

		public override void killMinecart(DamageSource p_94095_1_)
		{
			base.killMinecart(p_94095_1_);
			double var2 = this.motionX * this.motionX + this.motionZ * this.motionZ;

			if (!p_94095_1_.Explosion)
			{
				this.entityDropItem(new ItemStack(Blocks.tnt, 1), 0.0F);
			}

			if (p_94095_1_.FireDamage || p_94095_1_.Explosion || var2 >= 0.009999999776482582D)
			{
				this.explodeCart(var2);
			}
		}

///    
///     <summary> * Makes the minecart explode. </summary>
///     
		protected internal virtual void explodeCart(double p_94103_1_)
		{
			if (!this.worldObj.isClient)
			{
				double var3 = Math.Sqrt(p_94103_1_);

				if (var3 > 5.0D)
				{
					var3 = 5.0D;
				}

				this.worldObj.createExplosion(this, this.posX, this.posY, this.posZ, (float)(4.0D + this.rand.NextDouble() * 1.5D * var3), true);
				this.setDead();
			}
		}

///    
///     <summary> * Called when the mob is falling. Calculates and applies fall damage. </summary>
///     
		protected internal override void fall(float p_70069_1_)
		{
			if (p_70069_1_ >= 3.0F)
			{
				float var2 = p_70069_1_ / 10.0F;
				this.explodeCart((double)(var2 * var2));
			}

			base.fall(p_70069_1_);
		}

///    
///     <summary> * Called every tick the minecart is on an activator rail. Args: x, y, z, is the rail receiving power </summary>
///     
		public override void onActivatorRailPass(int p_96095_1_, int p_96095_2_, int p_96095_3_, bool p_96095_4_)
		{
			if (p_96095_4_ && this.minecartTNTFuse < 0)
			{
				this.ignite();
			}
		}

		public override void handleHealthUpdate(sbyte p_70103_1_)
		{
			if (p_70103_1_ == 10)
			{
				this.ignite();
			}
			else
			{
				base.handleHealthUpdate(p_70103_1_);
			}
		}

///    
///     <summary> * Ignites this TNT cart. </summary>
///     
		public virtual void ignite()
		{
			this.minecartTNTFuse = 80;

			if (!this.worldObj.isClient)
			{
				this.worldObj.setEntityState(this, (sbyte)10);
				this.worldObj.playSoundAtEntity(this, "game.tnt.primed", 1.0F, 1.0F);
			}
		}

		public virtual int func_94104_d()
		{
			return this.minecartTNTFuse;
		}

///    
///     <summary> * Returns true if the TNT minecart is ignited. </summary>
///     
		public virtual bool isIgnited()
		{
			get
			{
				return this.minecartTNTFuse > -1;
			}
		}

		public override float func_145772_a(Explosion p_145772_1_, World p_145772_2_, int p_145772_3_, int p_145772_4_, int p_145772_5_, Block p_145772_6_)
		{
			return this.Ignited && (BlockRailBase.func_150051_a(p_145772_6_) || BlockRailBase.func_150049_b_(p_145772_2_, p_145772_3_, p_145772_4_ + 1, p_145772_5_)) ? 0.0F : base.func_145772_a(p_145772_1_, p_145772_2_, p_145772_3_, p_145772_4_, p_145772_5_, p_145772_6_);
		}

		public override bool func_145774_a(Explosion p_145774_1_, World p_145774_2_, int p_145774_3_, int p_145774_4_, int p_145774_5_, Block p_145774_6_, float p_145774_7_)
		{
			return this.Ignited && (BlockRailBase.func_150051_a(p_145774_6_) || BlockRailBase.func_150049_b_(p_145774_2_, p_145774_3_, p_145774_4_ + 1, p_145774_5_)) ? false : base.func_145774_a(p_145774_1_, p_145774_2_, p_145774_3_, p_145774_4_, p_145774_5_, p_145774_6_, p_145774_7_);
		}

///    
///     <summary> * (abstract) Protected helper method to read subclass entity data from NBT. </summary>
///     
		protected internal override void readEntityFromNBT(NBTTagCompound p_70037_1_)
		{
			base.readEntityFromNBT(p_70037_1_);

			if (p_70037_1_.func_150297_b("TNTFuse", 99))
			{
				this.minecartTNTFuse = p_70037_1_.getInteger("TNTFuse");
			}
		}

///    
///     <summary> * (abstract) Protected helper method to write subclass entity data to NBT. </summary>
///     
		protected internal override void writeEntityToNBT(NBTTagCompound p_70014_1_)
		{
			base.writeEntityToNBT(p_70014_1_);
			p_70014_1_.setInteger("TNTFuse", this.minecartTNTFuse);
		}
	}

}